...
def files_path_collect_df(config: dict) -> "pd.DataFrame":
    ...
    Returns:
        pd.DataFrame: DataFrame con la información de los archivos de video encontrados y sus propiedades. Las columnas
        **file_creation_date**, **file_last_modified_date** y **file_scrap_date** son devueltas como columnas de tipo
        fecha (``datetime64[ns]`` en pandas) para facilitar filtrados y comparaciones cronológicas.
    ...
    try:
        ...
        video_target_folders: list = config.get("video_files_target_search_folder", [])
        video_exts: list = config.get("video_files_target_search_extension", [])
        if not video_exts:
        ...
        def _find_files_in_folders(root_path: str, target_folders: list, file_exts: list) -> pd.DataFrame:
            """Busca archivos con extensiones específicas dentro de carpetas de interés y devuelve un DataFrame"""
            results = []
        ...
        df_paths: pd.DataFrame = _find_files_in_folders(video_root_path, video_target_folders, video_exts)
        if df_paths.empty:
        ...
        for idx, file_path in enumerate(df_paths['video_file_path'], start=1):
            ...
            try:
                ...
                metadata = {
                    ...
                    "file_path": file_path,
                    # ─── Se devuelven objetos datetime, no cadenas ───
                ...
                metadata = {
                    "file_name": os.path.basename(file_path),
                    "file_path": file_path,
                    "file_creation_date": pd.NaT,
                    "file_last_modified_date": pd.NaT,
                    "file_scrap_date": datetime.now(),
                    "file_size_mb": 0,
                    "duration_hms": "00:00:00",
                    "duration_ms": 0,
            ...
            progress: float = (idx / total_files) * 100
        ...
        df_paths_properties: pd.DataFrame = pd.DataFrame(extracted_results)[[
            "file_name",
            "file_path",
            "file_creation_date",
        ...
        # Conversión explícita a datetime64[ns] para asegurar tipo fecha
        for col in ["file_creation_date", "file_last_modified_date", "file_scrap_date"]:
            df_paths_properties[col] = pd.to_datetime(df_paths_properties[col], errors="coerce")
        ...
        total_videos: int = len(df_paths_properties)
        process_duration: float = time() - start_time
